...
def lkDemo(img_path):
    ...
    pts, uv = opticalFlow(img_1.astype(np.float), img_2.astype(
        np.float), step_size=10, win_size=5)
    et = time.time()
    print("Time: {:.4f}".format(et - st))

    img = cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2RGB) / 255
    img = gaussianPyr(img, 1)[1]
    displayOpticalFlow(img, pts, uv)
...
def blendDemo():
    im1 = cv2.cvtColor(cv2.imread(folder + 'lion.jpg'),
                       cv2.COLOR_BGR2RGB) / 255
    im2 = cv2.cvtColor(cv2.imread(folder + 'grumpycat.jpg'),
                       cv2.COLOR_BGR2RGB) / 255
    mask = cv2.cvtColor(cv2.imread(folder + 'cl3mask.jpg'),
                        cv2.COLOR_BGR2RGB) / 255

    n_blend, im_blend = pyrBlend(im1, im2, mask, 5)
...
def main():
    print("ID:", myID())
    img_path = folder + 'boxman.jpg'
    lkDemo(img_path)
    pyrGaussianDemo(folder + 'naruto.jpg')
    pyrLaplacianDemo(folder + 'naruto.jpg')
    blendDemo()
